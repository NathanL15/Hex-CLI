...
2. Before claiming anything works:
   ...
   ```
   Full offline set = the step list in `.github/workflows/ci.yml` (26 files, 759 tests). No aggregate script exists; loop over `evals/test_*.py`. Each file is a standalone script with its own `TESTS` list and prints `N/N passed`; no pytest.
