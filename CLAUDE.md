...
to date through the 2.20.0 release on 2026-09-18 (code at
`__version__ = "2.20.0"`). When this file and the code disagree, the code
...
- Claims need evidence (2026-09-14): a behaviour claim in a finish (`_behaviour_claim`: ran it, works, responds, tests pass) needs a run_code/run_command this turn; read_file is inspection. One nudge, then the "Nothing was run this turn." notice. `verify_syntax` is a ladder and REPORTS its rung (`OK: parsed, cross-referenced` / `OK: parsed` / `NOT CHECKED`); html gets `tools.html_wiring_report`, python `tools.undefined_python_names`. Never make an unchecked file say OK. Cases `claims-1` (calculator, ceiling panel) and `claims-2`.
...
- A server that has served 1–2 h of eval traffic can degrade into sticky `ERROR_QUERY_FAILED (-6)`, which grades exactly like a model regression. Restart before every suite. Measured 2026-09-18 on three consecutive arms: the server's working set grows from ~3 GB to 6.3–6.7 GB over the hour, the machine falls to 1.2–1.6 GB free, and 13–16 of 245 runs go invalid on client timeouts (5–7 %, the `[PLATFORM]` flag), mostly late in the arm; gated cases that lose runs are re-run at six on a fresh server before the verdict is read. A background shell chain was killed by the low-memory watchdog under those conditions -- run release chains in the foreground, and only the arm itself detached. (Tested directly 2026-09-16: fresh blocks 45/48 vs 29/54/78-minute blocks 65/72, p = 0.74, canary and Rewind rates flat — the effect did not appear in 78 minutes, so a FAIL late in an arm is not explained by this alone.) The runner marks 5xx / URLError / -6 runs INVALID and refuses to start against a dead backend; a file with invalid runs is re-run, not compared.
...
- Patch = no model-facing change and nothing the launcher hands the server. Minor = features, any `prompts.py` change, launcher env or runtime keys, a `REQUIRED_NPURUN` bump. Major = model, runtime generation or history representation. 2.7.0 shipped 2026-09-12 (Terminal layout, persona fixes, installer, the two loop nudges and the named-file guard), 2.7.1 the same day (runner path fix), 2.8.0 that evening (installable package, `~/.shellai` data dir, Format-List classifier fix, README clips; on PyPI as `hexcli`); 2.8.1 the same night (installer icon repair, publish workflow, README rewrite, six-page paper); 2.9.0–2.11.1 followed on 09-13/14; 2.12.0 (item 1: the intent, contradiction and verification nudges, closest-path hint, Python-spelled actions, raw-JSON guard, long-write; paired A/B PASS) and 2.13.0 (AST command classifier, four alias bypasses closed, 0 of 119 recorded commands change tier) on 09-17; 2.14.0 (`write_file` decodes a body whose every quote is escaped; `runit-1` 5/10 → 20/20), 2.15.0 (`edit_file` refuses identical strings instead of saying "Edited") and 2.16.0 (`find_files` takes `pattern` as its glob) on 09-17/18, one arm and one pinned-gate PASS each; then, from the owner's 2026-09-18 HiLo/calculator session, 2.17.0 (the parser closes a reply that stops one brace short -- the root cause of that session's failure), 2.18.0 (a double-escaped `.py` body keeps a quote-adjacent `\n` when only that parses; cut from a branch off its release commit because CI failed on two unsorted imports -- read ruff's verdict line, not its last line), 2.19.0 (the intent nudge counts only a mutation that landed, reads "cannot be created" with any continuation, and catches a claimed fix with no edit) and 2.20.0 (`run_code` closes stdin and names the terminal command on `EOFError`), each with its own arm. The next release is 2.20.1 or 2.21.0 by the rule above.
...
- The REPL: one mode, 18 commands (`/help /clear /new /history /resume /search /diff /undo /stats /context /compact /memory /config /setup /tools /cwd /doctor /exit`), Esc cancels, Tab completes, a dim preview of the best slash-command match follows the caret while a command name is typed (`/he` shows `lp`; Right arrow accepts it with a trailing space; first match in `REPL_COMMANDS` order wins, so `/c` previews `/clear`; `lineedit._ghost`, 2026-09-13), did-you-mean for typos so a mistyped command never reaches the model. Keys (2026-09-13): Ctrl+Backspace = Ctrl+W (the console sends 0x7f for it; never bind 0x7f to a single backspace again), Ctrl+Delete kills the word forward, Ctrl+Home/End span a multi-line entry while Home/End stay on the line. Ctrl+Z/Ctrl+Y undo/redo (word-granular, `_handle` wraps `_handle_key`). A bare `/word` opens the command menu ABOVE the input row, between the top rule and the prompt (`_menu_rows`; rows below the input would push the caret up because the box is bottom-pinned; descriptions = `ui.COMMAND_HELP` parsed from HELP_TEXT; Up/Down pick, Tab takes, Enter runs, preview follows the pick). `/clear` and `/new` go through `_fresh_screen`: box DOWN via `suspend()` before printing (text written with the box up is conversation and lands at the bottom), `resume()` after; `/new` always prints the banner, `/clear` only when `live.banner_visible` (set by `banner_printed()`, cleared by a scrolling write, `note_scroll`, `screen_cleared`). Custom commands are `.md` files in `.shellai/commands/`. Chat/command modes and `/save /load /checkpoints /open /profile /model /models /mode` were removed in 2.3.0 (−685 lines); do not bring them back.
...
- `uc1-t3` (multi-turn: fix the runtime bug after seeing the traceback) sat at 1-3 of 3 across fresh servers (1/3 on 09-05, 3/3, 1/3, 2/3 on 09-12); every miss was three `edit_file` calls with a mismatched `old_string`, and the mismatch is the model rebuilding the lines from the traceback (`data` for `items`, dropped comment), not whitespace. 2026-09-13: `protocol_v2._apply_one` tier 4 is now a token-level three-way merge (file region, old_string, new_string): the model's hunks land where the file agrees with what it saw, misremembered tokens keep the file's text, conflicts refuse. The old ≥95 % paste of new_string is gone; it had installed `"average": average` over the file's `avg` in five of six uc1-t4 runs (harness-made NameError). Double-escaped `old_string` (`\"name\": \"demo\"`, 58 of 667 saved calls) is decoded once and retried; `write_file` decodes a one-line body of literal `\n`. Remaining uc1-t3 misses are runs that never call a tool. A 3/3 there is a lucky draw, not a baseline; the release rule's "no 3/3 lost" needs the case's rate, which means 6-run arms for the multi-turn suite too. The edit/write event lines name the tier (`, transfer match`, `, unescaped`) so an arm log shows which calls were rescued; `python evals/replay_edits.py` replays every saved uc1-t3/t4 edit attempt through the applier against the fixtures and is the offline check before any live arm (must report 0 wrong / 0 harmful; the fixed/applied counts are the lever's estimate).
...
| `CHANGELOG.md` | what shipped and the numbers | current through 2.20.0; no `Unreleased` section (add one with the next change) |
