...
def main() -> int:
    ...
    axes = [
        ...
        ("wall per task, extended suite", "lower", 20.0, "s",
         ...
         "median trace wall (Enter to final message, no harness overhead), paired runs same evening; v2.5.1 column uses the polling-on control. Ceiling: the 20 % of first calls that pay an in-line rebuild (PROMPT_LEVER.md §3.3) moved to a background prewarm; prompt length is not a lever (§3.2)."),
        ("energy per solved task, extended suite", "lower", 200.0, "J",
         {"v251": en_on["j_per_pass"], "v26": en_off["j_per_pass"], "lcpp": sm_lcpp["j_per_pass"], "ceiling": 180.0},
         f"system joules over the case window / passes; harness overhead included. llama.cpp value is from its smoke suite ({sm_lcpp['passes']} passes, 6 invalid). Ceiling: 2.6 minus the in-line rebuild share (~20 % of LLM time); prompt-length savings measured at zero (PROMPT_LEVER.md)."),
