...
def main() -> int:
    ...
    axes = [
        ...
        ("wall per task, extended suite", "lower", 20.0, "s",
         {"v251": ext_on["wall_s_med"], "v26": ext_off["wall_s_med"], "lcpp": None, "ceiling": 12.0},
         "median trace wall (Enter to final message, no harness overhead), paired runs same evening; v2.5.1 column uses the polling-on control. Ceiling: −40 % from a 1,200-token prompt (30 % decode tax) and fewer rebuilds."),
        ("energy per solved task, extended suite", "lower", 200.0, "J",
         {"v251": en_on["j_per_pass"], "v26": en_off["j_per_pass"], "lcpp": sm_lcpp["j_per_pass"], "ceiling": 150.0},
         f"system joules over the case window / passes; harness overhead included. llama.cpp value is from its smoke suite ({sm_lcpp['passes']} passes, 6 invalid). Ceiling: 2.6 minus prompt-length savings."),
        ...
        ("prompt tokens per task (input effectiveness)", "lower", 1500.0, "tokens",
         {"v251": ext_on["in_tok_med"], "v26": ext_off["in_tok_med"], "lcpp": None, "ceiling": 1300.0},
         "median estimated input tokens per run; the 2,331-token system prompt is most of it. Ceiling: V2 plan's ≤ 800-token core + tools."),
        ...
        ("decode at Hex context", "higher", 15.0, "tok/s",
         {"v251": agg(S, "npu", "main", "hex_turn2", "decode_tps", turn=2), "v26": agg(SA, "npu", "poll0", "hex_turn2", "decode_tps", turn=2),
          "lcpp": agg(S, "llamacpp", "main", "hex_turn2", "decode_tps", turn=2), "ceiling": 13.5},
         "turn-2 decode rate at ~2.7K tokens. Ceiling: decode-vs-context curve at a 1,300-token prompt."),
